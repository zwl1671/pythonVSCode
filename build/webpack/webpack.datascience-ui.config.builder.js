...
function getEntry(bundleType) {
    if (bundleType === 'notebook') {
        return {
    ...
    if (bundleType === 'renderers') {
        return {
            renderer: ['babel-polyfill', `./src/datascience-ui/renderers/index.tsx`],
        };
    }
...
function getPlugins(bundleType) {
    const plugins = [];
    if (isProdBuild) {
        plugins.push(...common.getDefaultPlugins(bundleType));
    }
...
function buildConfiguration(bundleType) {
    // Folder inside `datascience-ui` that will be created and where the files will be dumped.
    const bundleFolder = bundleType;
    const filesToCopy = [];
    if (bundleType === 'notebook' || bundleType === 'renderers') {
    ...
    return {
        ...
        //     moduleIds: 'hashed', // (doesn't re-generate bundles unnecessarily) https://webpack.js.org/configuration/optimization/#optimizationmoduleids.
            ...
            //         //     name: 'commons',
            //         //     chunks: 'initial',
            //         //     minChunks: bundleType === 'notebook' ? 2 : 1, // We want at least one shared bundle (2 for notebooks, as we want monago split into another).
